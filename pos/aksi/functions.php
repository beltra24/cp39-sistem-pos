...
// ========================================= PASAR ======================================= //
function tambahPasar($data) {
	global $conn;
	// ambil data dari tiap elemen dalam form
	$nama_pasar = htmlspecialchars($data['nama_pasar']);
	$alamat_pasar = htmlspecialchars($data['alamat_pasar']);
	

	// query insert data
	$query = "INSERT INTO pasar VALUES ('', '$nama_pasar', '$alamat_pasar')";
	mysqli_query($conn, $query);

	return mysqli_affected_rows($conn);
}

function editPasar($data) {
	global $conn;
	$id = $data["kode_pasar"];
...
function hapusPasar($id) {
	global $conn;
	mysqli_query( $conn, "DELETE FROM pasar WHERE kode_pasar = $id");
...
function tambahCustomer($data) {
	...
	$customer_email    = htmlspecialchars($data["customer_email"]);
		$kode_pasar = htmlspecialchars($data["kode_pasar"]);
   ...
   $query = "INSERT INTO relasi_pasarcustomer VALUES ('', '$kode_pasar', '$lastid')";
		mysqli_query($conn, $query);
...
function editCustomer($data){
  ...
  $cekrelasi= mysqli_num_rows(mysqli_query($conn, "select * from relasi_pasarcustomer where customer_id = '$id' "));

	if ( $cekrelasi > 0 ) {
	     $query2 = "UPDATE relasi_pasarcustomer SET kode_pasar = '$kode_pasar' WHERE customer_id = $id";
		mysqli_query($conn, $query2);
		$cekhasil = mysqli_affected_rows($conn);
		
	}else{
	     $query2 = "INSERT INTO relasi_pasarcustomer VALUES ('', '$kode_pasar', '$id')";
		mysqli_query($conn, $query2);
		$cekhasil = mysqli_affected_rows($conn);
	}
	...
	mysqli_query($conn,$query); 
	$cekhasil2 = mysqli_affected_rows($conn);
	if($cekhasil == "1" || $cekhasil2== "1"){
	    $cekhasilall = 1;
	}else{
	    $cekhasilall = 0;
	}
